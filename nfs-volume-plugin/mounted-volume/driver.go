...
func (p *Driver) Create(req *volume.CreateRequest) error {
	...
	status["args"] = args
	log.Printf("mounted-volume::driver::Create - options: %v",req.Options)
...
func (p *Driver) Mount(req *volume.MountRequest) (*volume.MountResponse, error) {
	log.Println("mounted-volume::driver::Mount - called Mount")
	p.m.Lock()
	defer p.m.Unlock()
	...
	log.Printf("mounted-volume::driver::Mount - executing: %s %v",p.mountExecutable,args)
	cmd := exec.Command(p.mountExecutable, args...)
	if out, err := cmd.CombinedOutput(); err != nil {
	        log.Printf("mounted-volume::driver::Mount - FAILED - command output %s",out)
		return &volume.MountResponse{}, fmt.Errorf("error mounting %s: %s", req.Name, err.Error())
	}
	createOption := getBooleanOption(volumeInfo.Options,"create")
	log.Printf("mounted-volume::driver::Mount - createOption: %v",createOption)
	if createOption == true {
		log.Printf("mounted-volume::driver::Mount - createOption enabled - using %s as sub-dir mount", req.Name)
		datavol := filepath.Join(mountPoint, req.Name)
		if err := createDest(datavol); err != nil {
			log.Printf("mounted-volume::driver::Mount - FAILED - creating datavol")
			return &volume.MountResponse{}, fmt.Errorf("error creating datavol %s: %s", req.Name, err.Error())
		}
		mountPoint = datavol
	}
	...
	log.Printf("mounted-volume::driver::Mount - mountpoint: %s", volumeInfo.MountPoint)
